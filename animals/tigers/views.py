...
def page_handler(request, id):
    ...
    photos = page.connecting_to_photos.all()
    paginator = Paginator(photos, 3)
    page_number = request.GET.get('page')
    photos_obj = paginator.get_page(page_number)
    #videos = Pages.objects.get(id__exact=id).videos_set.all()
    #paginator = Paginator(videos, 3)
    #page_number = request.GET.get('page')
    #videos_obj = paginator.get_page(page_number)
    return page, photos_obj
...
def news(request):
    page, photos_obj = page_handler(request, 5)
...
def program(request):
    page, photos_obj = page_handler(request, 1)
    return render(request, 'tigers/page_environment.html', {'page': page, 'photos_obj': photos_obj, })


def animal(request):
    page, photos_obj = page_handler(request, 2)
    return render(request, 'tigers/page_environment.html', {'page': page, 'photos_obj': photos_obj, })

def history(request):
    page, photos_obj = page_handler(request, 3)
    return render(request, 'tigers/page_environment.html', {'page': page, 'photos_obj': photos_obj,})


def premier(request):
    page, photos_obj = page_handler(request, 4)
    return render(request, 'tigers/page_environment.html', {'page': page, 'photos_obj': photos_obj, })
